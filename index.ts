...
import { browserData, voices, webglExt, shuffledCiphers } from './variables';




let cookie_counter = 0;
let akamaiSession: Electron.Session;
let akamaiSession2: Electron.Session;
let dataNum = 0;
// dataNum = '-1818464567'
...
app.on('ready', async () => {
	...
	async function get_abck(site, bmak, userAgent, ua_browser, proxy) {
        ...
        if (scriptUrl[1].includes("http")) endpoint = scriptUrl[1]
			...
			response.on('end', async () => {
				const cookies = await akamaiSession2.cookies.get({});
				console.log(cookies);
	...
	req.setHeader('sec-fetch-dest', 'script');
