...
const proxies = fs.readFileSync('./proxy.txt', 'utf-8').toString().toLowerCase().split("\r\n").filter(l => l.length !== 0);
import {app, session, net, BrowserWindow} from 'electron';
import {path as ghost_cursor} from 'ghost-cursor';
...
import {voices, webglExt, browserData} from './data'
console.error = (e) => {
...
let cookie_counter = 0,
	akamaiSession,
	akamaiSession2: Electron.Session
	const dataNum = 0;
	// dataNum = '-1818464567'
...
const jar = new CookieJar();
	const client = axios.create({
		httpsAgent: new HttpsCookieAgent({ jar, ciphers: shuffledCiphers }),
	});
app.on('ready', async () => {
	...
	async function controller(site: any, userAgent: string, ua_browser: string, proxy: string, abck: string, post_url: string, formInfo: any, xsrf){
		var bmak = await init(site, userAgent, ua_browser, proxy, abck, post_url, formInfo, xsrf);
		var initialCookie = await get_abck(site, bmak, userAgent, ua_browser, proxy);

	}
	

	async function init(site: any, userAgent: string, ua_browser: string, proxy: string | null, abck: string | null, post_url: string | null, formInfo: any, xsrf) {
		console.log('isABCK');
		console.log(abck);
	...
	const getScriptUrl = (data) => {
		...
		if (!scriptUrl) return;
		let endpoint;
		if (scriptUrl[1].includes("https")) endpoint = scriptUrl[1]
		else endpoint = site.url.slice(0, -1) + scriptUrl[1]
		return endpoint;
	}
	...
	async function get_abck(site, bmak, userAgent, ua_browser, proxy) {
		...
		try {
			...
			let req = await client.get(site.url, {
				headers: {
					...headers,
          ...
          if (cookiesF !== undefined) {
            for (const cookie of cookiesF) {
              await jar.setCookie(cookie, site.url, { ignoreError: true });
            }
          }
			...
			var xsrf = cookies.find(x => x.key === "frsx")?.value;
		...
		// 			const getCookies = await axios.get("https://www.zalando.pl/login", {
		// 					httpsAgent,
		// 					headers: {
		// 						...headers,
		...
		// 					}
		// 				});
		// 			const urlLogin = getCookies.request.res.responseUrl;
		// 			let requestId = getCookies.request.res.responseUrl.split("?")[1];
		// 			let cookies1;
		// 			if (getCookies.headers["set-cookie"] instanceof Array) {
		// 				cookies1 = getCookies.headers["set-cookie"].map((e) => Cookie.parse(e));
		// 			} else {
		// 				cookies1 = [Cookie.parse(getCookies.headers["set-cookie"])];
		// 			}
		// 			var abck = cookies.find(x => x.name === "_abck").value;
		// 			var xsrf = cookies.find(x => x.name === "frsx").value;
		// 			const csrf = cookies1.find((e) => e.key === "csrf-token").value;
		// 			getSubmitEndpoint(bmak, abck, ua_browser, userAgent, proxy, site, post_url, xsrf, csrf, requestId);
	...
	async function getSubmitEndpoint(bmak, abck, ua_browser, userAgent, proxy, site, post_url, xsrf){
		try {
			let req = await client.get(post_url, {
				headers: {
					...headers,
					'x-xsrf-token': xsrf,
					'referer': site.url,
			...
			const cookies = jar.toJSON().cookies;
			var xsrf1 = cookies.find(x => x.key === "frsx")?.value;
      console.log("🚀 ~ file: index.ts ~ line 373 ~ getSubmitEndpoint ~ xsrf1", xsrf1)
			...
			logger.red('exception ' + error.stack)
		}
	...
	// 			sensorGen(bmak, abck, ua_browser, userAgent, proxy, site, post_url, xsrf1, csrf, requestId);
	// 			// sensorGen(bmak, abck, ua_browser, userAgent, proxy, site, post_url);
	// 		}).catch((e) => logger.red('exception ' + e.stack));
	// 	});
	// });
	...
	async function validator(sensor, bmak, formInfo, userAgent, ua_browser, proxy, site, post_url, abck, xsrf) {
		...
		var bm_sz;
		try {
			const req = await client.post(post_url, `{\"sensor_data\":\"${sensor.toString()}\"}`, {
				headers: {
	...
	function get_cf_date(start?: boolean) {
		if (Date.now) {
